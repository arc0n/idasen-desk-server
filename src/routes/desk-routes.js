...
module.exports = function getRoutes(deskService) {
  ...
  router.post("/memory", async (req, res) => {
    const config = await getConfig().catch((err) =>
      res.status(400).send("no config found")
    );
    console.log("rec config", config);
    if (!config) return;
    ...
    if (
      ...
      Object.keys(req.body).some(
        (key) => !["position1", "position2", "position3"].includes(key)
      ) ||
      Object.values(req.body).some((val) => val > 60 || val < 7)
    ) {
      ...
      await saveConfig()
        .catch((err) => {
          res.status(500).send("writing config not possible");
        })
        .then(() => {
          res.send(true);
        });
