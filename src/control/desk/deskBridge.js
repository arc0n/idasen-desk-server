...
class DeskBridge extends EventEmitter {
    ...
    async scan() {
        ...
        try{
            await noble.startScanningAsync([], true, (err) => { // nothing
            }).catch((err) => {
        ...
        } catch (e) { throw new Error("Error while scanning for BT device")}
    ...
    handleError(err ,emitMessage) {
        if (err.message.includes("No compatible USB Bluetooth") || err.message.includes("state is unknown (not poweredOn)")) {
            if(emitMessage) {
